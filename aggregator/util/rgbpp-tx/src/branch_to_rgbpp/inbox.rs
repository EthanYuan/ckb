use crate::schemas::leap::Requests;
...
impl RgbppTxBuilder {
    pub fn create_add_queue_inbox_tx(
        &self,
        _request_height: u64,
        _request_tx: H256,
        requests_in_witness: Bytes,
    ) -> Result<H256, Error> {
        let requests_in_witness = Requests::from_slice(&requests_in_witness).map_err(|e| {
            Error::TransactionParseError(format!("get requests from witness error: {}", e))
        })?;
        let mut request_ids = vec![];
        let mut requests = vec![];
        for request in requests_in_witness {
            let request_id = request.as_bytes().pack().calc_raw_data_hash();
            request_ids.push(request_id);
            requests.push(request);
        }

        let (queue_inbox_cell, queue_inbox_cell_data) =
            self.get_rgbpp_queue_cell(self.rgbpp_queue_inbox_lock_key_path.clone())?;
        if !queue_inbox_cell_data.inbox().is_empty() {
            return Err(Error::QueueInboxHasUnprocessedRequests);
        }
        let existing_inbox = queue_inbox_cell_data
            .inbox()
            .as_builder()
            .extend(request_ids)
            .build();
        let queue_inbox_data = queue_inbox_cell_data
            .as_builder()
            .inbox(existing_inbox)
            .build();
        let queue_inbox_witness = Requests::new_builder().set(requests).build();

        // build inputs
        let inputs: Vec<CellInput> = std::iter::once(queue_inbox_cell.out_point.clone())
            .map(|out_point| {
        ...
        // build outputs
        let outputs = vec![queue_inbox_cell.output.clone().into()];
        let outputs_data = vec![queue_inbox_data.as_bytes().pack()];
        ...
        tx_builder
            ...
            .witness(
                WitnessArgs::new_builder()
                    .input_type(Some(queue_inbox_witness.as_bytes()).pack())
                    .build()
                    .as_bytes()
                    .pack(),
            );
        ...
        {
            let lock_script: Script = queue_inbox_cell.output.lock.clone().into();
            lock_groups
        ...
        change_builder.init(&mut tx_builder);
        {
            let queue_cell_input = TransactionInput {
                live_cell: queue_inbox_cell.clone().into(),
                since: 0,
            };
        ...
        TransactionSigner::new(&network_info)
            .sign_transaction(
                &mut tx_with_groups,
                &SignContexts::new_sighash(vec![message_queue_inbox_key, capacity_provider_key]),
            )
        ...
        info!("add queue inbox tx send: {:?}", tx_hash.pack());
